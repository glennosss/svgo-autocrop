...
module.exports = class AutocropUtils {
	...
	static plugin(ast, params, info) {
		...
		if (! AutocropUtils.translate(ast, params, viewboxNew) ) {
     ...
     */
	static translate(ast, params, viewboxNew) {
		let x = viewboxNew.x;
		let y = viewboxNew.y;
		if ( x==0 && y==0 || params.disableTranslate ) {
			return true; // Nothing to do.
		}

		// Attempt to translate back to (0, 0)
		try {
			new SvgTranslate(-x, -y, params.removeClass).translate(ast);
		} catch ( e ) {
			if (! params.disableTranslateWarning ) {
